...
def generate_article_md(article_info, images):
    """生成单篇文章的 Markdown 内容"""
    tags_str = ", ".join(f'"{t}"' for t in article_info['tags'])
...
"""
    ...
    if images and len(images) >= 1:
        content += f"![{images[0]['alt']}]({images[0]['path']})\n\n"

    content += f"**{article_info['description']}** 本文将从以下几个维度进行深入分析，帮助考生全面了解相关信息。\n\n"
    ...
    for i, heading in enumerate(article_info['headings']):
        ...
        if i == 1 and images and len(images) >= 2:
            content += f"![{images[1]['alt']}]({images[1]['path']})\n\n"
        
        content += f"关于「{heading}」，考生需要了解以下几个核心要点。\n\n"
        content += "首先，我们需要明确政策背景和考试要求。社区工作者考试作为基层岗位选拔的重要方式，其考试内容和评分标准都在不断规范化。考生在备考过程中，应当密切关注各地官方发布的最新公告，确保掌握第一手资讯。\n\n"
        content += "其次，从历年考试情况来看，提前系统复习是通过考试的关键。建议考生结合自身基础，制定切实可行的复习计划，合理分配各科目的复习时间。对于零基础考生，建议至少提前三个月开始准备，每天保证2-3小时的有效复习时间。\n\n"
        content += "再者，选对复习资料同样重要。市面上的社工考试教材种类繁多，建议优先选择最新版教材，并结合历年真题进行针对性练习。同时，可以关注各地人社局官网和社区服务网获取权威资讯。\n\n"

    # 结语
    content += "## 总结与建议\n\n"
    content += "综上所述，社区工作者考试虽然竞争日益激烈，但只要方法得当、准备充分，一次通过并非难事。建议考生在备考过程中，既要注重基础知识的掌握，也要关注政策动态和考试趋势。\n\n"
    content += "对于准备报考2026年社区工作者考试的考生，现在就是最好的开始时间。合理规划、坚持执行，相信每一位认真准备的考生都能收获理想的结果。\n\n"
    content += "---\n\n"
    content += "*本文由公考助手整理发布，更多社区工作者考试资讯请持续关注本站。*\n"
