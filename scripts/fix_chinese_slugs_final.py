...
# -*- coding: utf-8 -*-
"""
Fix Chinese slugs by renaming files to English-based slugs.
This fixes 500 errors on Vercel for static generation with Chinese characters.
"""
...
# Mapping table for Chinese phrases in filenames
SLUG_MAPPING = {
...
def rename_file_safely(old_path, new_path):
    ...
    if os.path.exists(new_path):
        print("  Warning: Target file already exists: " + os.path.basename(new_path))
        return False
    try:
...
def main():
    ...
    # Summary
    print("=" * 70)
    print("Total files renamed: " + str(len(renamed)))
    print("Total files skipped: " + str(len(skipped)))
    
    if skipped:
        print("\nSkipped files (no mapping):")
        for filepath, reason in skipped:
            print("  - " + os.path.basename(filepath) + ": " + reason)
    print("=" * 70)
    
    # Save log
    if renamed:
        log_file = "slug_fix_log_" + datetime.now().strftime('%Y%m%d_%H%M%S') + ".txt"
