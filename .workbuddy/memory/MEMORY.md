...
- **检查周期**：每日 07:30 检查10天前发布的内容（automation seo-2）
- **报告内容**：三引擎（百度/Bing/Google）+ Sitemap 收录状态，按引擎分行展示收录率
- **重要提示**：百度反爬较强，脚本返回 ❌ 不代表一定未收录；Google 国内网络受限，超时返回 —。建议手动在浏览器 `site:gk.edu-sjtu.cn` 二次确认

### 收录检查脚本升级（2026-05-22）
- **升级内容**：`seo_indexing_checker.py` 修复为真正的三引擎检查
  - 新增 `_do_search_check()` 通用方法
  - 新增 `check_indexing_baidu()` / `check_indexing_google()` 方法
  - `batch_check()` 对每篇文章逐一检查百度+Bing+Google（间隔2秒）
  - `generate_report()` 表格增加百度/Google列，统计按引擎分行展示收录率
- **修复前**：仅实际检查 Sitemap + Bing，百度和 Google 配置存在但从未被调用
...
| 2026-05-12 | 4篇 | 社区工作者招聘、上海招聘、待遇、报名条件 | ✅ 已推送 | ✅ Sitemap 100% / Bing 100% / 百度❌需手动确认 / Google—网络限制 |
...
## 发布后检查与修复（2026-05-22新增）

### 新增内容
所有10个文章发布自动化任务（seo-8-00/8-00-2/8-30/9-00/9-15/10-00/10-00-2/11-00/12-00/13-00）已统一添加"Step 6: 发布后检查与修复"步骤。

### 检查流程
1. **首页检查**：访问 https://gk.edu-sjtu.cn 确认首页正常
2. **新文章逐篇检查**：页面HTTP 200、日期YYYY-MM-DD格式（非时间戳）、正文完整、配图加载
3. **Sitemap检查**：确认新文章URL已收录

### 常见异常处理
- YAML解析错误 → 检查frontmatter格式，修复后重新push
- 日期显示时间戳 → 检查date字段引号，修复后重新push
- 图片无法加载 → 检查图片路径，修复后重新push
- 修复后必须重新验证直到全部正常
...
|-------|------|------|
| P0 | 删除3篇极高风险文章 | 待执行 |
| P1 | 更新自动化任务配置（内容比例+图片+发布后检查） | 已完成 |
